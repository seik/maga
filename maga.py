...
class Maga(asyncio.DatagramProtocol):
    ...
    def datagram_received(self, data, addr):
        ...
        try:
            ...
            self.send_message(
                data={"t": msg["t"], "y": "e", "e": [202, "Server Error"]}, addr=addr
            )
            raise e
    ...
    async def handle_query(self, msg, addr):
        ...
        if query_type == b"get_peers":
            ...
            tid = msg[b"t"]
            self.send_message(
                {"t": tid, "y": "r", "r": {"id": self.fake_node_id(node_id)}}, addr=addr
            )
    ...
    def ping(self, addr, node_id=None):
        self.send_message(
            {"y": "q", "t": "pg", "q": "ping", "a": {"id": self.fake_node_id(node_id)}},
            addr=addr,
        )
        ...
        self.send_message(
            {
                ...
                "y": "q",
                "q": "find_node",
                "a": {"id": self.fake_node_id(node_id), "target": target},
            },
            addr=addr,
        )
